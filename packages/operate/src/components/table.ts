...
export function createTable<T>(options: TableOptions<T>): {
	...
	el.className = "op-table-wrap"

	const header = document.createElement("div")
	header.className = "op-table-header"
	for (const col of options.columns) {
		const th = document.createElement("div")
		th.className = "op-table-th"
	...
	function setRows(rows: T[]): void {
		...
		if (rows.length === 0) {
			...
			empty.className = "op-table-empty"
		...
		for (const row of rows) {
			...
			if (options.onRowClick) {
				tr.classList.add("op-table-row--clickable")
				tr.addEventListener("click", () => options.onRowClick?.(row))
			}
			for (const col of options.columns) {
				const td = document.createElement("div")
				td.className = "op-table-td"
